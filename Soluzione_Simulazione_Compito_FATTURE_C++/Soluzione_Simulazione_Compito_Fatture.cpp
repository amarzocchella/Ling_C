...
g++ Soluzione_Simulazione_Compito_Fatture.cpp -o Archivio_Fatture
./Archivio_Fatture
...
...
void Ricerca() { // Funzione per cercare una voce
  ...
  ilMioFile.open(NOMEARCH.c_str(), ios_base::binary); // Apro il file
   ...
   string attributo;
  cout << endl;
  while (ilMioFile.read((char *) &fatturaInLettura, sizeof(fatturaInLettura))) {
   attributo = (string)fatturaInLettura.CodCliente;// converto il tipo char in string
  if (attributo.find(nomeDaCercare) != string::npos) { // Controllo se nella stringa codCliente è presente il campo di ricerca 

    cout << "\nTROVATO\n" << "\nCodice Cliente: " << fatturaInLettura.CodCliente << "\nCodice Fattura: " << fatturaInLettura.CodFattura << "\nData: " << fatturaInLettura.Data << "\nImporto: " << fatturaInLettura.Importo << "\nPagato: " << fatturaInLettura.Pagato << endl;
    
  }
    attributo = (string)fatturaInLettura.CodFattura;// converto il tipo char in string
  if (attributo.find(nomeDaCercare) != string::npos) { // Stessa cosa di prima solo con codFattura

    cout << "\nTROVATO\n" << "\nCodice Cliente: " << fatturaInLettura.CodCliente << "\nCodice Fattura: " << fatturaInLettura.CodFattura << "\nData: " << fatturaInLettura.Data << "\nImporto: " << fatturaInLettura.Importo << "\nPagato: " 
    << fatturaInLettura.Pagato << endl;
    
  }
  
   attributo = (string)fatturaInLettura.Data;// converto il tipo char in string
